	...
	$.fn.fancyTable = function(options) {
		...
		this.tableUpdate = function (elm) {
			...
			if(settings.pagination){
				...
				var prev =$("<a>",{
					html:'Previous',
					"data-n": 'Previous',
					style:"border:1px solid #CCCCCC; border-radius:0 !important	",
					...
					var pageN =$('.active-pagination').data("n");
					elm.fancyTable.page = pageN == 1 ? pageN : pageN-1;
				...
				var next =$("<a>",{
					...
					var pageN =$('.active-pagination').data("n");
					elm.fancyTable.page = pageN == elm.fancyTable.pages ? pageN : pageN+1;
					instance.tableUpdate(elm);
				});
		...
		this.each(function() {
			...
			if(settings.pagination && !settings.paginationElement){
				...
				$(elm).find("tfoot tr").append($("<td colspan='5' class='pag'></td>",{ }).attr("colspan",elm.fancyTable.nColumns));
			}
			instance.tableUpdate(elm);
